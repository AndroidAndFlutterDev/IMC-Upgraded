...
class ImcUpgradedCalculatorActivity : AppCompatActivity() {
    ...
    private var currentWeight: Int =
        40                               //We set a default weight (The user will change it)
    private var currentAge: Int =
        3                                   //We set a default age (The user will change it)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        enableEdgeToEdge()
        setContentView(R.layout.activity_imc_upgraded_calculator)
    ...
    private fun initListeners() {
        ...
        btnSubtractWeight.setOnClickListener {                           //Is the button for weight subtraction pressed?
            ...
            validateInput()      //Calling the validate input function for testing if the value entered by the user is valid (True or False)
        }
        ...
        btnSubtractAge.setOnClickListener {                             //Is the button for age subtraction pressed?
            ...
            validateInput()      //We call the validate input function for testing if the value entered by the user is value (True or False)
        }
        btnPlusAge.setOnClickListener {                                 //Is the button for age adding pressed?
            currentAge += 1      //We add 1 to the current age
            setAge()             //We call the setAge function
            validateInput()      //We call the validate input function for testing if the value entered by the user is valid (True or False)
        }
    ...
    private fun validateInput(): Boolean {
        if (currentAge <= 0 || currentWeight <= 0 || currentHeight <= 0) {
            val errorTextView: TextView = findViewById(R.id.tvError)
            errorTextView.text = getString(R.string.error_message)
    ...
    private fun navigateToResult(result: Double) {    //This is the function that will send the user to the next screen
        //We start the Activity with the Intent in it
        val intent = Intent(this, ResultImcUpgradedActivity::class.java)
    ...
    private fun calculateIMC(): Double {              //This is the function for the IMC calculation
        val imc =
            currentWeight / (currentHeight.toDouble() / 100 * currentHeight.toDouble() / 100)  //This is the formula, stored in an imc variable
    ...
    private fun setWeight() {                         //This is the function for the weight setting
        tvWeight.text =
            currentWeight.toString() //We convert the tvWeight to text so we convert the actual weight into a text, stored in the variable.
    }

    private fun setAge() {                           //This is the function for the age setting
        tvAge.text =
            currentAge.toString()       //We convert the tvAge to text so we convert the actual age into a text, stored in the variable.
    }
    ...
    private fun getBackgroundColor(isSelectedComponent: Boolean): Int {         //We set the color of any component, only if its selected
        ...
        val colorReference =
            if (isSelectedComponent) {    //We ask: is the component selected? And the result, we store it in a colorReference variable.
        ...
        return ContextCompat.getColor(
            this,
            colorReference
        )   //This will change the color according the ColorReference
    }
